...
IMAGE_DIR = 'C:/Users/baicol/Desktop/tianchi/part2/image_train/'     #图像所在
LABEL_DIR = 'C:/Users/baicol/Desktop/tianchi/part2/label_train/'         #标签所在
NUM_TEST = 200                  #测试集大小
...
def _gen_tfrecords(type,image_list,label_list):
    ...
    dic = _gen_dict()
    pic = tf.placeholder(tf.float32,[None])
    img = tf.image.resize_image_with_crop_or_pad(pic, 256, 32)
    with tf.Session() as sess:
        ...
        with tf.python_io.TFRecordWriter(tfrecord_path_name) as tfile:
            for i,image_name in zip(range(len(image_list)),image_list):
                try:
                    label_name = 'C:/Users/baicol/Desktop/tianchi/part2/label_train/'+image_name.split('/')[-1].split('.')[0]+'.txt'  #一个照片对应相应的label

                    sys.stdout.write('\n>已经保存：%d/%d' % (i+1,len(image_list)))
                    sys.stdout.flush()
                    image = np.array(Image.open(image_name))/255.0  #low
...
if __name__ =='__main__':
    if _tfrecord_exist(TFRCORD_FILE_DIR):
        ...
        train_image_paths_list = all_image_paths[:]
        train_label_paths_list = all_label_paths[:]
