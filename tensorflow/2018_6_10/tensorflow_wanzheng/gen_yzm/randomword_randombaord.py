...
def gen_one_yzm(char1,filename):
        ...
        img1 = Image.new(mode="RGB", size=(charlen, 32), color=(int(random.uniform(0,255)/5),int(random.uniform(0,105)*3),int(random.uniform(0,255)/4)))#随机底色
        # img1 = Image.new(mode="RGB", size=(charlen, 32), color=(0,0,0)) #白底
        draw1 = ImageDraw.Draw(img1, mode="RGB")

        # 每循环一次重新生成随机颜色
        color1 = (int(random.uniform(0,25)*10),int(random.uniform(0,25)/30),int(random.uniform(0,25)/4))

        # 把生成的字母或数字添加到图片上
        # 图片长度为120px,要生成5个数字或字母则每添加一个,其位置就要向后移动24px
        font1 = ImageFont.truetype("simkai.ttf", 30)  # 楷体
