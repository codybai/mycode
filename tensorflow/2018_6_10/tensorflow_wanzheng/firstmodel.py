...
class Global:
    def __init__(self):
        with open("F:/dataset/tianchi/[update] ICPR_text_train_part1_20180316/"
                  "train_1000/dataset.pkl", "rb+") as f:
            data = pk.load(f)
            x_ = []
            y = []
            self.j = 0  # 记录总的数据量
...
def create_model():
    ...
    x = conv(inputs, 3, 3, 3, 64, 'conv0')
    for i in range(1, 3):
        x = conv(x, 3, 3, 64, 128, 'conv'+str(i))

    for i in range(3,6):
        x = conv(x, 3, 3, 128, 256, 'conv' + str(i))
    x = max_pool(x)

    x = conv(x, 3, 3, 256, 512, 'conv' + str(i))
    x = conv(x, 3, 3, 256, 512, 'conv' + str(i))
