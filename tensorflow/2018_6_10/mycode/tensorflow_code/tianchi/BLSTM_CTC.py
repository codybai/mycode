...
class Global:
    def __init__(self):
        ...
        with open(path + "../util/dataset.pkl", "rb+") as f:
            data = pk.load(f)
            x_ = []
            y = []
            self.j = 0  # 记录总的数据量
...
#BLSTM+CTC
n_timesteps=256
in_lstm = tf.transpose(input,[0,2,1,3])
in_lstm = tf.reshape(in_lstm,[batch_size,n_timesteps,32*3])
result_conv8 = tf.transpose(in_lstm,[1,0,2])     #[time_steps,bs,512*1]
result_conv8 = tf.reshape(result_conv8,[n_timesteps*batch_size,32*3])
result_conv8 = tf.split(result_conv8,n_timesteps)      #[time_steps个[bs,512]] 为了满足blstm的输入要求
...
outputs = tf.reshape(outputs,shape=[n_timesteps*batch_size,2*num_hidden])
logits = tf.matmul(outputs,w_lstm)+b_lstm  #[time_steps*batch_size,num_classes]
logits = tf.reshape(logits,shape=[n_timesteps,batch_size,num_classes])
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):     #总共训练轮数
        ...
        g.test_i=0
        if epoch == 10000:
            saver.save(sess, "newmodel/model.ckpt")  # 保存
