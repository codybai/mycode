...
# conv2[16,128,32]
w_conv2 = tf.Variable(tf.truncated_normal(shape=[2, 2, 32, 64], stddev=0.1))
...
# conv3[8,64,64]
w_conv3 = tf.Variable(tf.truncated_normal(shape=[3, 3, 64, 128], stddev=0.1))
b_conv3 = tf.Variable(tf.constant(0.1, shape=[128]))
result_conv3 = tf.nn.relu(tf.nn.conv2d(pool_conv2, w_conv3, strides=[1, 1, 1, 1], padding='SAME') + b_conv3)
pool_conv3 = tf.nn.max_pool(result_conv3, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1], padding='SAME')
# conv4[4,64,128]
w_conv4 = tf.Variable(tf.truncated_normal(shape=[3, 3, 128, 256], stddev=0.1))
b_conv4 = tf.Variable(tf.constant(0.1, shape=[256]))
result_conv4 = tf.nn.relu(tf.nn.conv2d(pool_conv3, w_conv4, strides=[1, 1, 1, 1], padding='SAME') + b_conv4)
pool_conv4 = tf.nn.max_pool(result_conv4,ksize=[1,2,1,1],strides=[1,2,1,1],padding='SAME')
# conv5[2,64,256]
...
pool_conv5 = tf.nn.max_pool(result_conv5,ksize=[1,2,1,1],strides=[1,2,1,1],padding='SAME')
#[1,64,512]
w_conv6 = tf.Variable(tf.truncated_normal(shape=[3, 3, 512, 512], stddev=0.1))
b_conv6 = tf.Variable(tf.constant(0.1, shape=[512]))
result_conv6 = tf.nn.relu(tf.nn.conv2d(pool_conv5, w_conv6, strides=[1, 1, 1, 1], padding='SAME') + b_conv5)

w_conv7 = tf.Variable(tf.truncated_normal(shape=[3, 3, 512, 512], stddev=0.1))
b_conv7 = tf.Variable(tf.constant(0.1, shape=[512]))
result_conv7 = tf.nn.relu(tf.nn.conv2d(result_conv6, w_conv7, strides=[1, 1, 1, 1], padding='SAME') + b_conv5)

w_conv8 = tf.Variable(tf.truncated_normal(shape=[3, 3, 512, 512], stddev=0.1))
b_conv8 = tf.Variable(tf.constant(0.1, shape=[512]))
result_conv8 = tf.nn.relu(tf.nn.conv2d(result_conv7, w_conv8, strides=[1, 1, 1, 1], padding='SAME') + b_conv5)

bn1 = bn(result_conv8, 'BN1')




#[batch_size（100）,1,64,512]
bn1 = tf.transpose(bn1,[0,2,1,3]) #[bs,64,1,512]
bn1 = tf.reshape(bn1,[batch_size,64,1*512])   #time_steps = 64  [bs,64,512]
bn1=  tf.transpose(bn1,[1,0,2])     #[time_steps,bs,512]   [64,bs,512]
bn1 = tf.reshape(bn1,[time_steps*batch_size,512])
# bn1 = tf.split(bn1,time_steps)  #[time_steps,bs,512]
bn1 = tf.split(bn1,time_steps)      #[time_steps个[bs,512]] 为了满足blstm的输入要求

lstm_fw_cell = tf.nn.rnn_cell.GRUCell(num_hidden)
lstm_bw_cell = tf.nn.rnn_cell.GRUCell(num_hidden)
a = bn1
outputs,_,_=tf.nn.static_bidirectional_rnn(cell_fw=lstm_fw_cell,cell_bw=lstm_bw_cell,inputs=bn1,sequence_length=[time_steps]*batch_size,dtype=tf.float32)#output=[time_steps个[bs,2*num_hidden]]
