...
void MyTcpServer::incomingConnection(qintptr socketID)
{
    ...
    connect(th,SIGNAL(signal_inf(RADAR_PACKET)),this,SLOT(slotSingal_inf(RADAR_PACKET)));//array->RADAR_PACKET
...
void MyTcpServer::slotSingal_inf(RADAR_PACKET packet) //array->RADAR_PACKET
{
    emit signal_inf(packet); //array->packet
}
